...
class MenuMediator:
    def __init__(self, title: str, save_callable, open_callable, coord_callable, play_callable):
        self.do_save = save_callable
        self.do_open = open_callable
        self.do_coord = coord_callable
        self.do_play = play_callable
        ...
        self.label_url = Label(self.root, width=50, text=os.getcwd())
        self.label_url.grid(column=0, row=0, sticky=W, pady=5, padx=5)

        # text box for line input
        self.entry_url = Entry(self.root, width=50)
        self.entry_url.grid(column=0, row=1, sticky=W, pady=5, padx=5)
        ...
        self.button_save = Button(self.root, text=">> Play", command=lambda: self.do_play(self))
        self.button_save.grid(column=3, row=0, sticky=W+E, pady=5, padx=5, columnspan=1)

        # label with x,y
        self.label_coords = Label(
            self.root, text="x, y", width=10
        )
        self.label_coords.grid(column=1, row=1, sticky=W, pady=5, padx=5, columnspan=1)

        self.label_usage = Label(
            self.root, text="Press Ctrl+Alt to save coords"
        )
        self.label_usage.grid(column=2, row=1, sticky=W, pady=5, padx=5, columnspan=2)

        # big text area with scrollbar
        self.text_box = Text(self.root, height=40, width=75)
        self.text_box.grid(column=0, row=2, sticky=W+E, pady=5, padx=5, columnspan=4)
...
def _do_play(mediator):
    print('play')

def _main(argv):
    m = MenuMediator("test", _do_save, _do_open, _do_coords, _do_play)
    m.start()
    return 0
